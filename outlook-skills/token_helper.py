...
# scopes.json is part of the shipped code (single source of the scope
# contract), not user state — always resolved next to this script.
...
# Lock lives next to the actual token store (which may be outside the plugin
# tree — see _resolve_state_dir) so all writers contend on the same file.
LOCK_FILE = TOKEN_FILE.parent / (TOKEN_FILE.name + ".lock")
