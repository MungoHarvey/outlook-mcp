...
```bash
python3 scripts/graph_call.py POST "/me/messages" '{
    "subject": "SUBJECT",
    "body": {
      "contentType": "html",
...
```bash
python3 scripts/graph_call.py GET "/me/mailFolders/drafts/messages?$select=id,subject,toRecipients,createdDateTime,bodyPreview&$top=10&$orderby=createdDateTime desc"
```

## Delete a Draft

```bash
python3 scripts/graph_call.py DELETE "/me/messages/{draftId}"
```
