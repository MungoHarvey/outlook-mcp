...
```bash
python3 scripts/graph_call.py POST "/me/messages" '{"subject":"SUBJECT","body":{"contentType":"text","content":"BODY"},"toRecipients":[{"emailAddress":{"address":"to@example.com"}}]}'
```
...
```bash
python3 scripts/graph_call.py PATCH "/me/messages/{draftId}" '{"subject":"Updated Subject","body":{"contentType":"text","content":"Updated body"}}'
```
...
```bash
python3 scripts/graph_call.py POST "/me/messages/{draftId}/send"
```
