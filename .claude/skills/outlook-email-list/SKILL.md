...
```bash
python3 scripts/graph_call.py GET "/me/messages?$select=id,subject,from,toRecipients,receivedDateTime,bodyPreview,hasAttachments,importance,isRead&$top=10&$orderby=receivedDateTime desc"
```
...
```bash
python3 scripts/graph_call.py GET "/me/messages?$search=\"SEARCH_TERM\"&$select=id,subject,from,receivedDateTime,bodyPreview,isRead&$top=10"
```
