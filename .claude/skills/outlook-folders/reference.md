...
```bash
python3 scripts/graph_call.py GET "/me/mailFolders/{parentFolderId}/childFolders?$select=id,displayName,totalItemCount,unreadItemCount"
```
...
for MSG_ID in id1 id2 id3; do
  python3 scripts/graph_call.py POST "/me/messages/$MSG_ID/move" "{\"destinationId\": \"TARGET_FOLDER_ID\"}"
done
```
