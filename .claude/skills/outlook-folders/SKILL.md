...
```bash
python3 scripts/graph_call.py GET "/me/mailFolders?$select=id,displayName,totalItemCount,unreadItemCount"
```
...
```bash
python3 scripts/graph_call.py POST "/me/mailFolders" '{"displayName":"FolderName"}'
```

Subfolder:
```bash
python3 scripts/graph_call.py POST "/me/mailFolders/{parentFolderId}/childFolders" '{"displayName":"SubfolderName"}'
```
...
```bash
python3 scripts/graph_call.py POST "/me/messages/{messageId}/move" '{"destinationId":"TARGET_FOLDER_ID"}'
```
