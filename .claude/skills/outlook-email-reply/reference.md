...
```bash
python3 scripts/graph_call.py POST "/me/messages/{messageId}/reply" '{"comment":"<p>Thanks for this — <b>great work</b>!</p>"}'
```
...
```bash
python3 scripts/graph_call.py POST "/me/messages/{messageId}/forward" '{
...
DRAFT=$(python3 scripts/graph_call.py POST "/me/messages/{messageId}/createReply" '{"comment":"See attached"}')
DRAFT_ID=$(echo "$DRAFT" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])")

# Step 2: Add attachment
python3 scripts/graph_call.py POST "/me/messages/$DRAFT_ID/attachments" '{
...
# Step 3: Send
python3 scripts/graph_call.py POST "/me/messages/$DRAFT_ID/send" '{}'
```
