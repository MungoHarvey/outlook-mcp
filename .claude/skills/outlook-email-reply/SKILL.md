...
```bash
python3 scripts/graph_call.py POST "/me/messages/{messageId}/reply" '{"comment":"Reply body text here"}'
```

## Reply All

```bash
python3 scripts/graph_call.py POST "/me/messages/{messageId}/replyAll" '{"comment":"Reply body text here"}'
```

## Forward

```bash
python3 scripts/graph_call.py POST "/me/messages/{messageId}/forward" '{"comment":"FYI — see below","toRecipients":[{"emailAddress":{"address":"recipient@example.com"}}]}'
```
